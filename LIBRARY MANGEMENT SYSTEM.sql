...
WHERE I.ISSUE_DATE BETWEEN '2023-06-01' AND '2023-06-30'; -- 8

SELECT BOOK_TITLE 
FROM BOOKS
WHERE BOOK_TITLE LIKE '%history%'; -- 9

SELECT BRANCH_NO, COUNT(EMP_ID) AS Total_Employees
FROM EMPLOYEE
GROUP BY BRANCH_NO
HAVING COUNT(EMP_ID) > 5; -- 10

SELECT E.EMP_NAME AS MANAGER_NAME, B.BRANCH_ADDRESS
FROM EMPLOYEE E
JOIN BRANCH B ON E.Emp_Id = B.MANAGER_ID; -- 11

SELECT C.Cus_Name AS Customer_Name
FROM CUSTOMER C
JOIN ISSUESTATUS I ON c.Cus_Id = I.Issued_Cus
JOIN BOOKS B ON I.ISBN_Book = B.ISBN
WHERE B.Rental_Price > 25; -- 12
